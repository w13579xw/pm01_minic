...
All the environments are registered in the `engineai_rl_lab` extension. They start
with `EngineAI` in their name.
"""
    ...
    for info in pkgutil.iter_modules(path, prefix):
        ...
        if info.ispkg:
            try:
                ...
                print(f"Import error: {info.name}: {e}")
...
def import_packages():
    sys.path.insert(0, f"{pathlib.Path(__file__).parent.parent}/source/engineai_rl_lab/engineai_rl_lab/tasks/")
...
def main():
    """Print all environments registered in `engineai_rl_lab` extension."""
    # print all the available environments
    table = PrettyTable(["S. No.", "Task Name", "Entry Point", "Config"])
    table.title = "Available Environments in EngineAI RL Lab"
    ...
    for task_spec in gym.registry.values():
        if "EngineAI" in task_spec.id and "Isaac" not in task_spec.id:
